...
const pets_reducer = (state, action) => {
  ...
  if(action.type === GET_PETS_SUCCESS) {
    const featured_pets = action.payload.filter((pet) => pet.featured === true)
    return {...state, pets_loading: false, pets: action.payload, featured_pets}
  }
  ...
  if(action.type === GET_SINGLE_PET_BEGIN) {
    return {...state, single_pet_loading: true, single_pet_error: false}
  }

  if(action.type === GET_SINGLE_PET_SUCCESS) {
    return {...state, single_pet_loading:false, single_pet:action.payload}
  }

  if(action.type === GET_SINGLE_PET_ERROR) {
    return {...state, single_pet_error:true, single_pet_loading:false}
  }

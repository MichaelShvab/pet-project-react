...
const Filters = () => {
  ...
  }, updateFilters, clearFilters, all_pets} = useFilterContext()

  const categories = getUniqueValues(all_pets, 'type')
  const companies = getUniqueValues(all_pets, 'name')
  const colors = getUniqueValues(all_pets, 'breed')
        ...
        <div className="form-control">
          ...
          <select name="company" value={company} onChange={updateFilters} className='company'>
            {companies.map((c, index) => {
              return <option key={index} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>
            })}
          </select>
        </div>
        ...
        <div className="form-control">
          ...
          <div className="colors">
            {colors.map((c, index) => {
              ...
              return <button key={index} className={`${color === c ? 'color-btn active' : 'color-btn'}`} name='color' data-color={c} style={{background: c}} onClick={updateFilters}>{color === c ? <FaCheck /> : null}</button>
            })}
          </div>
        </div>
